...
Runtime image: Debian slim + stripped `ceps-client-mcp` + bundled `tests/wasm` (`CEPS_WASM_ROOT=/opt/ceps/tests/wasm`). Default entrypoint is stdio; pass `--http` for `:6790`.

```bash
make docker-build-mcp IMAGE_TAG=local
docker run --rm -p 6790:6790 ceps-client-mcp:local --http --listen 0.0.0.0:6790
```
...
docker pull interchouette/ceps-client-mcp:dev
```
