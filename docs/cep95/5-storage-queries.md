...
Ownable owner for tip `OwnedCEP95` uses path `[1, 1]` (Odra 1-based module field indices: `ownable`, then `owner`). `get_owner()` reads that `state` item; `transfer_ownership(new_owner)` calls the Ownable entrypoint. Public `get_approved` maps on-chain `approved_for`.
